    ...
    private var debugOptionsCard: some View {
        SettingsCard {
            VStack(spacing: 4) {
                ...
                Divider()
                SettingsToggle(isOn: nativeSettingsManager.crashForensicsMode(true).projectedValue, icon: "stethoscope", label: "Crash Forensics Mode", infoMessage: "Keeps richer diagnostics in game session logs, including guest and stub logs on supported launch profiles. Keep ON while debugging crashes or scene soft-locks.")
                Divider()
                SettingsToggle(isOn: nativeSettingsManager.eastwardSceneForensicsMode(false).projectedValue, icon: "scope", label: "Eastward Scene Forensics", infoMessage: "Eastward-only diagnostics profile. Keeps guest/stub logs enabled and turns on debug logs (trace remains off) to capture scene transition issues.")
                Divider()
